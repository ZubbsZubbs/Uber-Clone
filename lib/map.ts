...
export const calculateDriverTimes = async ({
  ...
  try {
    const timesPromises = markers.map(async (marker) => {
      // Fetching route from driver to user
      ...
      if (!dataToUser.routes || dataToUser.routes.length === 0) {
        console.warn("No routes found to user location.");
        return { ...marker, time: null, price: null }; // Gracefully handle
      }

      const timeToUser = dataToUser.routes[0]?.legs[0]?.duration?.value; // Time in seconds

      // Fetching route from user to destination
      ...
      if (!timeToUser || !timeToDestination) return { ...marker, time: null, price: null };

      // Calculating total time and price
